...
module.exports.create_academy = async (req, res) => {
    ...
    const newAcademy = new Academy({ name: data.name, description: data.description });
...
module.exports.create_academy_user = async (req, res) => {
    ...
    if (academy == null) {
        ...
        const user = new User({ username: data.username, password: await utils.getPasswordBcrypt(data.password), role: "academy", academyId: academy._id })
        await user.save();
